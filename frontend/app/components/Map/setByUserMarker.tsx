...
const SetByUserMarker = ({mapMove, userAddress, setUseCenterPosition}: 
    { mapMove: boolean, userAddress: any, setUseCenterPosition: Function }) => {
    const icon = useRef<HTMLDivElement>(null);
    const markerText = useRef<HTMLDivElement>(null);
    const [markerWidth, setMarkerWidth] = useState('40px')
    ...
    return (
        <div style={{width: markerWidth}} className={`marker ${mapMove || !userAddress || userAddress.length < 2 ? 'load' : ''}`}>
            ...
            {  !mapMove && userAddress  && userAddress.length > 1 ?  <span ref={markerText}> { userAddress.length > 1 ? userAddress : '' } </span> : '' }
        </div>
    )
};
